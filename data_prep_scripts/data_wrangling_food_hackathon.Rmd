...
#identifying the columns for which we have no info at the moment
   ...
   #write out as a csv
   write.csv(data_alghny_vendor_loc,"data_prep_scripts/algh_county_wic_vendor_loc_cleaned.csv",
             row.names = F)
...
data_algh_summer_sites <- dat_mod %>% bind_rows((addr_day_time_df %>% bind_cols(algh_summer_sites %>%select(latitude=x,longitude=y,zip_code=zip,location_description=additional, name = site))))
#write out as csv
