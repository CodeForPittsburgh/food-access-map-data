...
## create empty dataframe according to data model; keep data types as character for ease
dat0 <- as_tibble(data.frame(matrix(nrow=0, ncol=length(dat_mod_col_names)))) %>% 
  mutate_all(as.character)

colnames(dat0) <- dat_mod_col_names

#  ---------------------------------SNAP
# first, manually unzip the SNAP data to extract the csv.
dat1 <- "SNAP_Store_Locations.csv"
SNAP <- read_csv(paste0(read_loc2, dat1), col_types =  cols(.default = col_character()))
...
pfpc_green_grocer <- readxl::read_excel(paste0(read_loc1, dat2), col_types =  "text")
...
WIC <- read_csv(paste0(read_loc2, dat3), col_types =  cols(.default = col_character()))
...
growpgh <- readxl::read_excel(paste0(read_loc1, dat4), col_types =  "text")
