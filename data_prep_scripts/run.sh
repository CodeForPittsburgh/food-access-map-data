...
cat food-data/new-datasets/wicresults.json  | jq --arg current_time "$current_time" '.timestamp |= $current_time' > tmp.json && mv tmp.json food-data/new-datasets/wicresults.json
...
#Merge duplicate rows, resolving conflicts on critical information by prioritizing some data sources, outputing final result into "merged_datasets.csv"		
		Rscript data_prep_scripts/auto_merge_duplicates_wrapper.R "data_prep_scripts/source_field_prioritization.csv" > food-data/processed-datasets/merged_datasets.csv
