...
* Recommend wait to assign UID until all compiled, then check name and address -- this is unique key
...
multi_day_info <- multi_day_info %>% select(weekday, FID) %>% mutate(value = 1) %>% tidyr::spread(key = "weekday", value = "value") %>% replace_na(, 0)
...
agh_farm_markets <- dat1 %>% 
  bind_rows(agh_farm_markets %>% 
              ...
              mutate(type = "Farmers Market",
                     ...
                     open_time = time_calc(Day_Time)$open_time,
                     close_time = time_calc(Day_Time)$close_time,
                     date_from = season_calc(Season)$date_from,
                     date_to = season_calc(Season)$date_to) %>% ## this may not be robust
              select(type, weekday_val, open_time, close_time, date_from, date_to,
