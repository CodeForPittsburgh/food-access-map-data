...
data_alghny_vendor_loc <-  dat_mod %>% bind_rows(algh_vendor_loc %>% mutate(state="PA",
                                                                            WIC = 1) %>% ## cgm add 07/02
                                                   select(name,address,city=municipa_1,zip_code=zip_codes,state))
...
07/15/2019 see other data prep script/new summer sites data source (for 2019 from USDA; updates weekly on Wednesdays; has API). 
...
# data_algh_summer_sites <- dat_mod %>% 
#   bind_rows((addr_day_time_df %>% 
#                bind_cols(algh_summer_sites %>%
#                            select(latitude=x,
#                                   longitude=y,
#                                   zip_code=zip,
#                                   location_description=additional, 
#                                   name = site,
#                                   open_to_spec_group = open_close)))) ## cgm 07/02 add
# #write out as csv
# write.csv(data_algh_summer_sites,"agh_summer_sites_cleaned.csv",row.names = F)
